...
internal actor KnockEnvironment {
    ...
    func setPublishableKey(key: String) throws {
        guard key.hasPrefix("sk_") == false else {
            ...
            Knock.shared.log(
                type: .error, category: .general, message: "setPublishableKey", status: .fail,
    ...
    public func setDeviceToken(_ token: String?) async {
        let previousTokens: [String] = getPreviousPushTokens()
        if let token: String = token, !previousTokens.contains(token) {
...
extension Knock {
    ...
    public func setUserInfo(userId: String?, userToken: String?) async {
        await environment.setUserInfo(userId: userId, userToken: userToken)
    }

    public func setUserInfo(userId: String?, userToken: String?, completion: @escaping () -> Void) {
        Task {
            await environment.setUserInfo(userId: userId, userToken: userToken)
            completion()
        }
    ...
    public func getUserId(completion: @escaping (String?) -> Void) {
        Task {
            completion(await environment.getUserId())
        }
    }

    public func getDeviceToken() async -> String? {
        await environment.getDeviceToken()
    }

    public func getDeviceToken(completion: @escaping (String?) -> Void) {
        Task {
            completion(await environment.getDeviceToken())
        }
    }

    public func getPushChannelId() async -> String? {
        await environment.getPushChannelId()
    }

    public func getPushChannelId(completion: @escaping (String?) -> Void) {
        Task {
