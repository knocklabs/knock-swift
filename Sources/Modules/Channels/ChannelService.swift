...
internal class ChannelService: KnockAPIService {
    ...
    func getUserChannelData(channelId: String) async throws -> Knock.ChannelData {
        try await get(path: "/users/\(KnockEnvironment.shared.getSafeUserId())/channel_data/\(channelId)", queryItems: nil)
    }
    
    func updateUserChannelData(channelId: String, data: AnyEncodable) async throws -> Knock.ChannelData {
        let body = ["data": data]
        return try await put(path: "/users/\(KnockEnvironment.shared.getSafeUserId())/channel_data/\(channelId)", body: body)
    }
}
